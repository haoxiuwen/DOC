...
| **Certificate Name** | String | 是       | 配置为 FCM 的发送者 ID。<ul><li>对于新版证书，你可以在 [Firebase 控制台](https://console.firebase.google.com)的 **Project settings** > **Cloud Messaging** 页面查看 **Sender ID** 参数的值。</li><li>对于旧版证书，你需在 [Firebase 控制台](https://console.firebase.google.com)的 ***Project settings > Cloud Messaging** 页面中，在 **Cloud Messaging API (Legacy)** 区域中获取发送者 ID 。</li></ul><br/> 证书名称是声网即时通讯服务器用于判断目标设备使用哪种推送通道的唯一条件，因此必须确保你[在即时通讯 IM 中集成 FCM 时设置的 Sender ID](#initialization)与这里设置的一致。 |
